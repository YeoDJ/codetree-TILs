...
bool moveKnight(int idx, int dir) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (isMoved[i] || i == target || arr[i].hp <= 0)
                continue;
            // 기사가 충돌하지 않는 경우(두 기사의 좌표가 떨어진 경우)
            if (arr[i].spos.first > n_arr[target].epos.first || n_arr[target].spos.first > arr[i].epos.first)
                continue;
            if (arr[i].spos.second > n_arr[target].epos.second || n_arr[target].spos.second > arr[i].epos.second)
                continue;
...
int main() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        cin >> idx >> dir;
        idx--;
        if (arr[idx].hp > 0 && moveKnight(idx, dir))
            for (int j = 0; j < n; j++)
                arr[j].hp -= dmg[j];
    }
